/**
 * 마크다운 데이터 + 실제 파일 시스템을 기반으로 musicTracks.json 생성
 * 
 * 사용법: npx tsx scripts/generate-music-tracks-from-markdown.ts
 */

import * as fs from "fs";
import * as path from "path";

// 마크다운 데이터 (사용자가 제공한 전체 데이터)
const markdownData = `
# All Songs
...
// musicMetadata.json 로드
let musicMetadata: any = null;
try {
  const metadataPath = path.join(process.cwd(), "src", "lib", "music", "musicMetadata.json");
  musicMetadata = JSON.parse(fs.readFileSync(metadataPath, "utf-8"));
} catch (e) {
  console.error("musicMetadata.json을 읽을 수 없습니다:", e);
}

// 실제 파일 시스템에서 파일 찾기 (musicMetadata.json 사용)
function findActualFiles(genre: string, title: string): { mp3: string | null; image: string | null } {
  const musicsDir = path.join(process.cwd(), "public", "musics", genre);
  const imagesDir = path.join(process.cwd(), "public", "musics_img", genre);
  ...
  // musicMetadata.json에서 매핑 찾기
  if (musicMetadata && musicMetadata.tracks) {
    const match = musicMetadata.tracks.find((track: any) => {
      const normalizedTrackTitle = normalizeString(track.title);
      return normalizedTrackTitle === normalizedTitle && track.genre === genre;
    });
    
    if (match) {
      mp3File = match.fileName;
      // 이미지는 실제 파일 시스템에서 찾기 (musicMetadata의 imageFileName은 숫자 형식이므로)
    }
  }
  
  // musicMetadata에서 찾지 못한 경우 파일 시스템에서 직접 찾기
  if (!mp3File && fs.existsSync(musicsDir)) {
    const mp3Files = fs.readdirSync(musicsDir).filter(f => f.endsWith(".mp3"));
    for (const file of mp3Files) {
      const normalizedFile = normalizeString(file.replace(".mp3", ""));
  ...
  // 이미지 파일 찾기 (musicMetadata에서 찾지 못한 경우)
  if (!imageFile && fs.existsSync(imagesDir)) {
    ...
    for (const file of imageFiles) {
      ...
      if (normalizedFile === normalizedTitle || normalizedTitle === normalizedFile) {
        imageFile = file;
        break;
      }
    }
  ...
  return { mp3: mp3File, image: imageFile };
}
...
Object.entries(byGenre).forEach(([genre, blocks]) => {
  ...
  blocks.forEach((block) => {
    ...
    // 실제 파일 찾기
    const actualFiles = findActualFiles(genre, block.title);
    
    if (!actualFiles.mp3) {
      warnings.push(`⚠️  MP3 파일을 찾을 수 없음: ${genre}/${block.title}`);
    }
    if (!actualFiles.image) {
      warnings.push(`⚠️  이미지 파일을 찾을 수 없음: ${genre}/${block.title}`);
    }
    
    // PNG 파일명: 마크다운의 하이픈을 공백으로 변환하여 실제 파일명과 매칭
    // 마크다운: "A-glass-of-soju.png" → 실제 파일: "A glass of soju.png"
    const pngFileNameFromMarkdown = block.png.split("/")[1]; // "A-glass-of-soju.png"
    const actualImageFileName = actualFiles.image || pngFileNameFromMarkdown.replace(/-/g, " "); // 하이픈을 공백으로
    
    tracks.push({
      musicID,
      genre,
      title: block.title,
      mp3Url: actualFiles.mp3 ? `/musics/${genre}/${actualFiles.mp3}` : `/musics/${genre}/${block.mp3.split("/")[1]}`,
      imageUrl: `/musics_img/${genre}/${actualImageFileName}`,
      artist: block.artist,
