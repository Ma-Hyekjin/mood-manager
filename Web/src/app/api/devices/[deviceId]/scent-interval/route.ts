 ...
 * [파일 역할]
 * - 센트 분사 주기 변경 API
 * - PUT: 센트 디바이스의 분사 주기 업데이트
 *
 * [사용되는 위치]
 * - 디바이스 관리 페이지에서 센트 분사 주기 조절 시 사용
 *
 * [주의사항]
 * - 인증이 필요한 엔드포인트
 * - manager 또는 scent 타입 디바이스만 가능
 * - 분사 주기는 5, 10, 15, 20, 25, 30분 중 하나 (기본값 30분)
 */
 ...
 *
 * 요청:
 * - interval (required): 분사 주기 (5, 10, 15, 20, 25, 30 중 하나, 단위: 분)
 *
 * 응답:
 * - 성공: { device: Device }
...
export async function PUT(
  ...
  try {
    ...
    const validation = validateRequiredFields(body, ["interval"]);
    if (!validation.valid) {
      return NextResponse.json(
        {
          error: "INVALID_INPUT",
          message: "분사 주기는 필수 입력 항목입니다.",
        },
        { status: 400 }
      );
    }
    ...
    if (!validIntervals.includes(interval)) {
      return NextResponse.json(
        {
          error: "INVALID_INPUT",
          message: "분사 주기는 5, 10, 15, 20, 25, 30분 중 하나여야 합니다.",
        },
        { status: 400 }
      );
    }
    ...
    // 6. 디바이스 타입 검증 (manager 또는 scent만 가능)
    if (device.type !== "manager" && device.type !== "scent") {
      return NextResponse.json(
        {
          error: "INVALID_DEVICE_TYPE",
          message:
            "센트 분사 주기는 Manager 또는 Scent 타입 디바이스에만 설정할 수 있습니다.",
        },
        { status: 400 }
      );
    }
    ...
    console.error(
      "[PUT /api/devices/:deviceId/scent-interval] 센트 분사 주기 변경 실패:",
      error
    );
    return NextResponse.json(
      {
        error: "INTERNAL_ERROR",
        message: "센트 분사 주기 변경 중 오류가 발생했습니다.",
      },
      { status: 500 }
    );
  }
