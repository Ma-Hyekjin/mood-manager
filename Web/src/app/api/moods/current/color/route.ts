...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    // 5. 조명 컬러 변경이 필요한 디바이스만 업데이트 (Manager, Light)
    ...
    // 6. Preset 업데이트 (updatedType = 'color')
    ...
    console.error("[PUT /api/moods/current/color] Error:", error);
    return NextResponse.json(
      { error: "INTERNAL_ERROR", message: "Failed to update color" },
      { status: 500 }
    );
  }
}
