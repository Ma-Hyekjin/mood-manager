 ...
 *
 * [사용되는 위치]
 * - 프론트엔드에서 센트만 변경할 때 호출
 *
 * [주의사항]
 * - 인증이 필요한 엔드포인트
 * - 센트 변경으로 인한 무드 업데이트
 * - 관련 디바이스(Manager) 상태 자동 업데이트
 * - 같은 무드명의 다른 센트 버전으로 변경
 */
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    // 5. 센트 변경이 필요한 디바이스만 업데이트 (Manager, Scent)
    ...
    // 6. Preset 업데이트 (updatedType = 'scent')
    ...
    console.error("[PUT /api/moods/current/scent] Error:", error);
    return NextResponse.json(
      { error: "INTERNAL_ERROR", message: "Failed to update scent" },
      { status: 500 }
    );
  }
}
