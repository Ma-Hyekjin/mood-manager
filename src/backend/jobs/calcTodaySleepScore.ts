...
export async function calcTodaySleepScore(userId: string) {
  // 1) Firestore에서 오늘 분석 대상 raw 데이터 가져오기
  const raw = await fetchTodaySleepRaw(userId);

  // 분석할 데이터가 하나도 없는 경우
  if (raw.length === 0) {
    return { score: null as number | null, reason: "NO_DATA" as const };
  }
  ...
  if (sessions.length === 0) {
    return { score: null as number | null, reason: "NO_SESSION" as const };
  }
